...
import { rankRowSx } from "../theme/modernTheme";
...
export function LeaderboardsPage() {
  ...
  if (!validId) {
    return (
      <Alert sx={{ m: 2 }}>
        Invalid season id. <Link to="/leaderboards">Standings</Link>
      </Alert>
    );
  }
  ...
  return (
    <Stack spacing={2} sx={{ p: 2 }}>
      ...
      <Typography variant="body2">
        <Link to="/leaderboards">All standings</Link>
        {" · "}
        <Link to="/live">Live scores</Link>
        {auth?.status === "authenticated" && auth.me.role === "President" ? (
          <>
            {" · "}
      ...
      {mode === "live" ? (
        <Alert severity="warning">
          Live / Unfinalized — standings and points may change until the president finalizes events.
        </Alert>
      ) : null}
      ...
      <Table size="small">
        ...
        <TableBody>
          {(data?.teams ?? []).map((t, i) => (
            <TableRow key={t.teamId} sx={rankRowSx(i + 1)}>
      ...
      <Table size="small">
        ...
        <TableBody>
          {(data?.malePlayers ?? []).map((p, i) => (
            <TableRow key={p.playerId} sx={rankRowSx(i + 1)}>
              <TableCell>{i + 1}</TableCell>
